function  err  = errorH1dPG( f, gradF, u4e, c4n, n4e, kU, baseU, gradU, degreeF)
%ERRORH1DPG Computes the H^1 error err = |f-u| for the solution u to the 
...
%                         for f
%
% output:   err         - the error |f-u| in the H^1 norm
%
...
INT = @(n4p, Gpts4p, Gpts4ref)...
   (f(Gpts4p)-eval(Gpts4ref, nElem, nBaseU, baseU, u4e)).^2;
...
err = err + sum(integrate(INT, c4n, n4e, (degree-1)^2, area4e));
err = sqrt(err);
end

function sumsum = eval(Gpts, nElem, nBaseU, base, u4e)
% computes u on each element 
% (sums the base functions with the weight given by u4e)
sumsum = zeros(nElem,1);
for k=1:nBaseU
sumsum = sumsum + u4e(:,k).*base{k}(Gpts);
end
end

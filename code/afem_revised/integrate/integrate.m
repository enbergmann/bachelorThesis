function val = integrate(integrand, c4n, n4p, degree, OPTsize4p)
...
% return value must have the dimension [ nParts n m ]
%
% integrate.m
...
%         OPTsize4p - length4s (1D) or area4e (2D) (Optional parameter)
% output: val           - integral value of dimension [ nParts n m ]
%
% [val of dimension (nParts x n x m)] = integrand(n4p,Gpts4p,Gpts4ref)
...
% Modified (P. Bringmann): parallelization of for-loop over all
% GaussPoints, compute dimension of output vector in advance

    %% Number of Parts
    nParts = size(n4p,1);

    %% Number of Gauss Points
    nGpts = ceil((degree+1)/2);

    %% 1D or 2D
    if size(n4p,2) == 2
        % 1D
        [Gpts4ref,Gwts4ref] = getGaussPoints(nGpts);
        if nargin == 5
            weightFactor = OPTsize4p;
        else
    ...
    elseif size(n4p,2) == 3
        % 2D triangle
        [Gpts4ref,Gwts4ref] = getConProdGaussPoints(nGpts);
        if nargin == 5
            weightFactor = 2*OPTsize4p;
        else
    ...
    % Compute dimension of output vector
    firstGpt4p = ref2arbitrary(c4n,n4p,Gpts4ref(1,:));
    dim = size(integrand(n4p,firstGpt4p,Gpts4ref(1,:)));

    if numel(dim) < 3; dim(3) = 1; end
    val = zeros(nParts,dim(2),dim(3));
    % Integrate
    for curGpt = 1:nGpts
        %transformation
        curGpt4p = ref2arbitrary(c4n,n4p,Gpts4ref(curGpt,:));
        % evaluate function handle eval must be [nParts n m]
        integrandVal = integrand(n4p,curGpt4p,Gpts4ref(curGpt,:));
        % weighted sum
        curGwt4p = weightFactor*Gwts4ref(curGpt);
        curGwt4p = reshape( curGwt4p*ones(1,dim(2)*dim(3)),[nParts,dim(2),dim(3)]);

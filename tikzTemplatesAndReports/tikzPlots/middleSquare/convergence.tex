\documentclass[tikz]{standalone}
\usepackage{amsmath,amsfonts,amssymb,amsthm,mathtools}
\usepackage{tikz}
\usepackage{pgfplots, pgfplotstable}

\pgfplotsset{compat=newest,
	axis background/.style={fill=black!5!white},
	grid style={densely dotted,semithick}
  }

%\pgfplotstableset{
%  create on use/test/.style={
%    create col/copy column from table={glebExactEnergyDifference1e-2.txt}{0}
%    }
%  }

\begin{document}

% TODO find here how csv files work
    % \addplot [dashed, black, thick, mark = diamond] 
    % table[col sep = comma, x = {nrDof}, y = {error4lvl}] 
    % {lvlOutput.csv}; 

\pgfplotstableset{col sep = comma, %space, (for txt files, than it works great)
  % create on use/nrDof1e-2/.style={create col/copy column from 
  % table={lvlOutputEps1e-2.txt}{nrDof}},
  % create on use/nrDof1e-6/.style={create col/copy column from 
  % table={lvlOutputEps1e-6.txt}{nrDof}}
}


\begin{tikzpicture}
  \begin{loglogaxis}[legend pos = outer north east, 
    scale = 2, xlabel = {nrDof},
    ymajorgrids=true,
    legend entries = {$\theta = 0.5$,
                     $\theta = 1$,
                     $\eta$,
                     $\eta_\text{Vol}$,
                     $\eta_\text{Jumps}$}]
    \addlegendimage{solid, mark=*}
    \addlegendimage{densely dashed, mark=square*}
    \addlegendimage{no markers, green}
    \addlegendimage{no markers, red}
    \addlegendimage{no markers, orange}

    % adaptive 0.5
    \addplot [solid, green, mark = *] 
    table[x = {nrDof}, y = {eta}] 
    {1e-4/adaptive/lvl16/lvlOutput.csv}; 

    \addplot [solid, red, mark = *] 
    table[x = {nrDof}, y = {etaVol}] 
    {1e-4/adaptive/lvl16/lvlOutput.csv}; 

    \addplot [solid, orange, mark = *] 
    table[x = {nrDof}, y = {etaJumps}] 
    {1e-4/adaptive/lvl16/lvlOutput.csv}; 

    % uniform
    \addplot [densely dashed, green, mark = square*] 
    table[x = {nrDof}, y = {eta}] 
    {1e-4/uniform/lvl8/lvlOutput.csv}; 

    \addplot [densely dashed, red, mark = square*] 
    table[x = {nrDof}, y = {etaVol}] 
    {1e-4/uniform/lvl8/lvlOutput.csv}; 

    \addplot [densely dashed, orange, mark = square*] 
    table[x = {nrDof}, y = {etaJumps}] 
    {1e-4/uniform/lvl8/lvlOutput.csv}; 
    
    % % adaptive 0.999
    % \addplot [densely dotted, cyan, mark = square*] 
    % table[x = {nrDof}, y = {error4lvl}] 
    % {1e-5/uniform/lvl7/lvlOutput.csv}; 

    % \addplot [densely dotted, green, mark = square*] 
    % table[x = {nrDof}, y = {eta}] 
    % {1e-5/uniform/lvl7/lvlOutput.csv}; 

    % \addplot [densely dotted, red, mark = square*] 
    % table[x = {nrDof}, y = {etaVol}] 
    % {1e-5/uniform/lvl7/lvlOutput.csv}; 

    % \addplot [densely dotted, orange, mark = square*] 
    % table[x = {nrDof}, y = {etaJumps}] 
    % {1e-5/uniform/lvl7/lvlOutput.csv}; 

    % \addplot [densely dotted, gray, mark = square*] 
    % table[x = {nrDof}, y = {diffGlebExactE}] 
    % {1e-5/uniform/lvl7/lvlOutput.csv}; 

    % eta uniform
    add slope triangle
    \draw (axis cs:8e5,4e-2) 
                 --(axis cs:8e5,1.26e-1) node [midway, right] {\scriptsize$1$}
                 -- (axis cs:8e4,1.26e-1) node [midway, above] {\scriptsize$2$}
                 -- cycle
                 ;

    % eta adaptive
    add slope triangle
    \draw (axis cs:8e5,1.5e-3) 
                 --(axis cs:8e5,1.5e-2) node [midway, right] {\scriptsize$1$}
                 -- (axis cs:8e4,1.5e-2) node [midway, above] {\scriptsize$1$}
                 -- cycle
                 ;
  \end{loglogaxis}
\end{tikzpicture}

%\begin{tikzpicture}
%  \begin{loglogaxis}
%    \addplot
%    table[x={nrDof}, y={error4lvl}]  
%    {f01/epsModerate/lvl15_nrDof1209309/lvlOutput.txt}
%  \end{loglogaxis}
%\end{tikzpicture}

\end{document}


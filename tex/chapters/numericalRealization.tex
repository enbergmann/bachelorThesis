...
\begin{algorithmic}\\
  ...
  $v_0\coloneqq 0$ in $\textup{CR}^1_0(\mathcal T)$,
  $\Lambda_0\in P_0(\mathcal{T};
  \overline{B(0,1)}),\tau>0$  
  \For{$j = 1,2,\dots$}
  \State $\tilde{u}_j\coloneqq u_{j-1}+\tau v_{j-1}$,
  \State $\Lambda_j\coloneqq
...
$$
in $\textup{CR}^1_0(\mathcal{T})$ for solution $u_j\in\textup{CR}^1_0(\mathcal{T})$.
\State
  \State $v_j\coloneqq\frac{1}{\tau}(u_j-u_{j-1})$\\
  \EndFor
...
To realize Algorithm \ref{alg:PrimalDualIteration} numerically the data structures \texttt{c4n}, 
\texttt{n4e}, \texttt{s4e}, \texttt{n4s}, \texttt{n4sDb}, \texttt{n4sNb}, \texttt{s4n},
and \texttt{area4e}
are recapitulated, 
see also \cite{Carstensen2017script} and section 7.8 of \cite{CarstensenBrennerFEM}.\\
Let $\mathcal{T}$ be a regular triangulation of the polyhedral bounded
Lipschitz domain $\Omega$ into triangles with set of nodes $\mathcal{N}$ and set of edges
...
as follows.\\
The $j$-th row \texttt{c4n(j,:)=[$x$ $y$]} of \texttt{c4n} contains the coordinates of the $j$-th 
node $z_j=(x,y)$  
of $\mathcal T$.

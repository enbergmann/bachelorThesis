...
Sattelpunktsproblem \todo{überlegen, ob ich das wirklich, wie Bartels,
Sattelpunktsproblem nennen möchte oder einfach von einem 'äquivalenten' Problem
reden will} her. Wir betrachten die konvexe Menge 
\begin{align*}
  K
  \coloneqq 
...
P_0\left(\Tcal;\Rbb^2\right)\subset L^\infty(\Omega;\Rbb^2)$ können wir damit
das Funktional
...
\begin{align}
  ...
  \leq \Enc(\vcr).
\end{align}
Weiterhin gilt, wenn wir $\Lambda_0\in P_0\left(\Tcal;\Rbb^2\right)\cap K$ mit
der Signumfunktion aus \Cref{eq:signumFunction} elementweise auf allen
...
\end{equation}
\todo[inline]{Kopiere sowas wie die eine zu löschende Zeile unten, in der
auf beiden Seiten noch das Infimum ergänzt wird, hier hoch}
Insbesondere ist damit die Funktion $\ucr\in\CR^1_0(\Tcal)$ aus der Lösung
$\left( \ucr,\bar\Lambda_0 \right)\in\CR^1_0(\Tcal)\times
P_0\left(\Tcal;\Rbb^2\right)$ des folgenden Sattelpunktsproblems
\todo{siehe oben} eine Lösung
von \Cref{prob:discreteProblem}.

\begin{problem}\label{prob:discreteSaddlepointProblem}
  Finde $\left( \ucr,\bar\Lambda_0 \right)\in\CR^1_0(\Tcal)\times
...
\begin{proof} 
  ...
  \todo[inline]{Lösche von hier\ldots}
  ...
  \todo[inline]{\ldots bis hier.}
  \todo[inline]{Es muss noch argumentiert werden, dass die erste Komponente
  des Sattelpunkts gleich $\tilde{u}_\CR$ ist (folgt aus der Eindeutigkeit der
  Lösung und daraus (nochmal überlegen), dass jede erste Komponente eines 
  Sattelpunkts Lösung des diskreten Problems ist}
  \ldots nach \ldots existiert eine Sattelpunkt (,), d.h. es gilt
  \begin{align*}
    \inf_{\vcr\in\CR^1_0(\Tcal)}\sup_{\Lambda_0\in P_0\left(\Tcal;\Rbb^2\right)} 
    L(\vcr,\Lambda_0)
    =
    ...
    L(\vcr,\Lambda_0).
  \end{align*}
  Somit ist $\left(\tilde{u}_\CR,\bar\Lambda_0\right)\in 
  ...
  Minimierer von $L(\bullet, \bar\Lambda_0)$ in $\CR^1_0(\Tcal)$ ist und
  ...
  Da $\bar\Lambda_0$ Maximierer von $L\!\left(\tilde{u}_\CR,\bullet\right)$ in 
  ...
  Die Funktion $\bar\Lambda_0\in P_0(\Tcal;\Rbb^2)$ erfülle
  ...
  \end{align*}
  Um zu beweisen, dass $\tilde{u}_\CR$ \Cref{prob:discreteProblem} löst, genügt es
  aufgrund der Eindeutigkeit der Lösung $\ucr$ von
  \Cref{prob:discreteProblem} zu zeigen, dass $\tilde{u}_\CR=\ucr$ in
  $\CR^1_0(\Tcal)$.
  Es gilt
  \begin{align*}

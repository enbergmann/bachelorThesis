...
die konvex-konkave Sattelfunktion (nach \cite[Section 33]{Roc70}) 
...
\begin{equation*}
  ...
  \sup_{\Lambda_0\in P_0\left(\Tcal;\Rbb^2\right)}L(\vcr,\Lambda_0),
\end{equation*}
das heißt für eine Lösung $\left( \tilde{u}_\CR,\bar\Lambda_0
\right)\in\CR^1_0(\Tcal)\times P_0\left(\Tcal;\Rbb^2\right)$ des folgenden
Minimaxproblems gilt für die nach \Cref{thm:discreteProblemExistenceUniqueness} 
eindeutige Lösung $\ucr\in\CR^1_0(\Tcal)$ von \Cref{prob:discreteProblem}, dass
$\tilde{u}_\CR = \ucr$ in $\CR^1_0(\Tcal)$.
\todo{eindeutige Lösung und damit eindeutigkeit der ersten Kopmponente des
Sattelpunkts (s. Beweis unten) hier schon erwähnen oder lieber nicht?
Entsprechend auch ucr oder tilde ucr im Problem hier}

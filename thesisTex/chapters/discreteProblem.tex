...
und $\Lambda\in P_0(\Tcal;\Rbb^2)\subset L^\infty(\Omega;\Rbb^2)$ können
wir damit das Funktional
$L:\CR^1_0(\Tcal)\times  P_0(\Tcal;\Rbb^2)\to [-\infty,\infty)$ 
definieren durch
...
$\Lambda\in P_0(\Tcal;\Rbb^2)\cap K$, dass
\begin{align*}
...
Insgesamt folgt
\begin{align*}
  \sup_{\Lambda\in P_0(\Tcal;\Rbb^2)}L(\vcr,\Lambda)=
  \sup_{\Lambda\in P_0(\Tcal;\Rbb^2)\cap K}L(\vcr,\Lambda)
  \leq \Enc(\vcr).
\end{align*}

Weiterhin gilt für alle $\vcr\in\CR^1_0(\Tcal)$, wenn wir
$\Lambda\in P_0(\Tcal;\Rbb^2)\cap K$ elementweise auf allen $T\in\Tcal$
...
$\Enc(\vcr)\leq\sup_{\Lambda\in P_0(\Tcal;\Rbb^2)}L(\vcr,\Lambda)$.
...
\begin{problem}\label{prob:discreteSaddlepointProblem}
  Finde $\ucr\in\CR^1_0(\Tcal)$ und $\Lambda\in P_0(\Tcal;\Rbb^2)$, sodass
  \begin{align*}
    L(\ucr,\Lambda) 
    = 
    \inf_{\vcr\in\CR^1_0(\Tcal)}\sup_{\Lambda\in P_0(\Tcal;\Rbb^2)}
    L(\vcr,\Lambda).
  \end{align*}
\end{problem}
...
äquivalente Charakterisierungen für diese zeigen.
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item[(ii)] Es existiert ein
      $\bar\Lambda\in P_0(\Tcal;\Rbb^2)$ mit
...
\begin{proof} 
  ...
  $\Enc(\vcr) = \sup_{\Lambda\in P_0(\Tcal;\Rbb^2)}L(\vcr,\Lambda)$
  für alle $\vcr\in\CR^1_0(\Tcal)$, folgt,
  dass $\bar\Lambda\in P_0(\Tcal;\Rbb^2)\cap K$ {\color{red} (denn
  sonst ist das innere $\sup$ nicht erfüllt, da sonst
  $-I_K (\bar\Lambda)=-\infty$})
  existiert mit 
  \begin{align*}
  ...
  \begin{align*}
    ...
    L(\vcr,\Lambda)
    \geq 
    \sup_{\Lambda\in P_0(\Tcal;\Rbb^2)} \inf_{\vcr\in\CR^1_0(\Tcal)} 
    L(\vcr,\Lambda),
  \end{align*}
  folgt insgesamt
  \begin{align*}
  ...
  \CR^1_0(\Tcal)\times ( P_0(\Tcal;\Rbb^2)\cap K)$ nach \cite[S. 380,
  Lemma
  36.2]{Roc70} Sattelpunkt 
  von $L$ bezüglich der Maximierung über $ P_0(\Tcal;\Rbb^2)$ und
  ...
  Maximierer von $L(\ucr,\bullet)$ über $ P_0(\Tcal;\Rbb^2)$.
  ...
  $ P_0(\Tcal;\Rbb^2)$ maximiert wird, wird das konvexe Funktional 
  $-L(\ucr,\bullet)$ 
  \todo[inline]{zitieren, was konkav ist und das -konvex=konkav}
  von $\bar\Lambda$ in $ P_0(\Tcal;\Rbb^2)$ minimiert 
  ...
  \end{align*}
  da $\bar\Lambda\in K$.
  Für $\Lambda\in  P_0(\Tcal;\Rbb^2)\cap K$ folgt insbesondere
  \begin{align*}
    (\gradnc\ucr,\Lambda-\bar\Lambda)_{L^2(\Omega)} 
    &\leq 0,\quad\text{also }\\ 

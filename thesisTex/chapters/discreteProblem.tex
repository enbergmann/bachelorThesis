...
$L^2$-Fehler zwischen den Minmierern $u_\C\in S^1(\Tcal)$ und
...
Mit den Betrachtungen in \Cref{sec:discreteProblemFormulation} 
...
\begin{definition}[Verfeinerungsindikator]
  ...
  Für $d\in\mathbb{N}$ (in dieser Arbeit $d=2$) und $0<\gamma\leq 1$ definieren
  wir für alle $T\in\Tcal$ die Funktionen
  \begin{align*}
    \eta_\text{V}(T)
    &\coloneqq
    ...
    \eta_\text{J}(T)
    &\coloneqq
    |T|^{\gamma/n}\sum_{F\in\Ecal(T)}\Vert [\ucr]_F\Vert_{L^1(F)}.
  \end{align*} 

...
$L^2$-Fehler für $\epsstop=10^{-2}$ ab circa $10^4$ Freiheitsgraden bei $2\cdot
10^{-2}$ und es lässt sich erahnen, dass dieser Effekt auch für
$\epsstop=10^{-3}$ ab ungefähr $10^6$ Freiheitsgraden bei $4\cdot 10^{-3}$
einsetzt. 
Da für $\epsstop\in\{10^{-4},10^{-5}\}$ bis $10^6$ Freiheitsgrade dieses
...
Bei vielen Freiheitsgraden, das heißt bei kleinen Netzweiten, muss
...
$10^{-4}$ und $10^{-5}$ in \Cref{fig:parEpsStopConvergence} kaum unterscheiden,
die Wahl $\epsstop=10^{-5}$ aber eine wesentlich längere Laufzeit des Programms
verursacht, begründen wir damit unsere Wahl $\epsstop=10^{-4}$.
Es bleibt aber anzumerken, dass der Verfeinerungsindikator $\etaT$ für keine
der Wahlen von $\epsstop$ stagniert. 
%TODO
Dies liegt womöglich daran, dass in
seinen Anteilen $\etaV$ und $\etaJ$ die

Nach Def des Verfeinerungsindikators gilt
\begin{align*}
  \etaT 
  \leq
...
\end{align*}
Das heißt obwohl nach Interpretation des ROF-Modells $\Vert f-\alpha\ucrt^2$
stagniert für $\epsstop=10^{-2}$ nach unserer Beobachtung hier und 
$\sum_{F\in\Ecal}\Vert[\ucrt]_F\Vert_{L^1(F)}$ ebenfalls wahrscheinlich 
stagniert (vgl.  \Cref{fig:f01JumpTerms} aus
\Cref{sec:experimentsWithExactSolution}), werden weiterhin Dreiecke markiert
und damit wird $\max_{T\in\Tcal}\{|T|,|T|^{\gamma/2}\}$ (stimmt das? falls
ja, in der Ungleichung eben vielleicht direkt so schreiben) weiterhin kleiner,
womit $\etaT$ nicht stagniert.
Dann sehen wir in den Bildern TODO (a) und (b), das vor Stagnation des Fehler
die Verfeinerung von $\epsstop=10^{-2}$ und $\epsstop=10^{-4}$ bei gleichen
Freiheitsgraden von etwa \ldots ähnlich aussehen,
in (c) und (d) sehen wir dann aber, dass für $\epsstop =10^{-2}$, mehrere
Level nach Stagnationsstart, bei nun \ldots Freiheitsgraden mehr
der Flächeninhalt der Dreicke eine größere Rolle zu spielen scheint im
Vergleich zu $10^{-4}$. 
...
%\section{Experimente mit bekannter exakter Lösung und Konvergenz der Iteration}
\label{sec:experimentsWithExactSolution}
...
Bild als weiteres Indiz, nur eben mit $\alpha$}

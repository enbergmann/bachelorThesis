...
mit $\partial A$ das innere von $A$ mit $\interior(A)$.
...
$\Pbb_1(\Tcal,\Rbb^m)$.
Für ein Dreieck $T\in\Tcal$ sei der Mittelpunkt $\Mid(T)$ und die längste
Seitenlänge $h_T$.
...
$\nu_F$. 
Für eine Innenkante $F\in\Fcal(\Omega)$ bezeichnen wir dann die beiden
Dreiecke, die $F$ als gemeinsame Kanten haben, so mit $T_+$ und $T_-$, dass
$\nu_{T_+}$ und $\nu_F$ gleichorientiert sind, also $\nu_{T_+}\cdot\nu_F=1$.

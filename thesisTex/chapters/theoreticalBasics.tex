...
In \cite{Zei85} werden die folgenden Aussagen auf reellen lokal konvexen
Räumen $X$ formuliert.
Da nach \cite[S. 781, (43)]{Zei86} alle Banachräume 
(\glqq B-spaces\grqq \cite[S.  786]{Zei86}) lokal konvex sind und in dieser
Arbeit die Aussagen nur auf Banachräumen benötigt werden, beschränke ich
die folgenden Aussagen auf reelle Banachräume.
Im folgenden sei $(X,\Vert\bullet\Vert_X)$ stets ein reeller Banachraum und
...
\begin{theorem}[\protect{\cite[S. 387, Proposition 47.13]{Zei85}}]
  ...
  Sei $F: X\to [-\infty,\infty]$ konvex und G\^{a}teaux-differenzierbar an der 
  Stelle $u\in X$ mit G\^{a}teaux-Differential $F'(u)$.

  Dann gilt $\partial F(u)=\{F'(u)\}$.
  \todo[inline]{TODO wie bei B-space suche G-derivative in den Zeidlers
  und checke Notation und Definition mit der
  Gateaux/Frechet-differenzierbarkeit, für die ich mich entschieden haben
  werde, zitiere insbesondere wieder, wo G-derivate klar als Gateaux 
...
\begin{theorem}[\protect{\cite[S. 389, Theorem 47.B]{Zei85}}]
  ...
  ein $u_0\in X$ mit $F_k(u_0)<\infty$ für alle $k\in\{1,2\ldots,n\}$, sodass
  $F_1,F_2,\ldots,F_{n-1}$ stetig an der Stelle $u_0$ sind.
...
\begin{theorem}[\protect{\cite[S. 396f., Definition 47.15, Theorem
  ...
  \begin{align*}
    \langle u^\ast-v^\ast,u-v\rangle\geq 0\quad \text{für alle}\quad u,v\in X, 
...
\begin{definition}
  ...
  dem Raum aller (vektoriellen) Radon Maße. Dabei wird die Anwendung

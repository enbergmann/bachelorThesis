...
\begin{remark}
  %TODO
  \todo[inline]{TODO schreibe die äquivalenz und Bedingungen genauer, was soll
  hier wirklich gesagt werden? Vielleicht auch erst beim Diskreten Problem,
  da es dort Nullranddaten gibt? Beachte insbesondere, dass $g=\alpha f$ (oder
  sowas) in Bartels und behandel also das noch richtig hier}
  In \cite[Kapitel~10.1.3]{Bar15} wird \Cref{prob:continuousProblem} formuliert
  mit dem Funktional 
  \begin{align*}
    I(v)\coloneqq |v|_{\BV(\Omega)} + \frac{\alpha}{2}\int_\Omega (v-f)^2\dx
  \end{align*}
  für $v\in \BV(\Omega)\cap L^2(\Omega)$.
  Da 
  $I(v) = E(v) - \Vert v\Vert_{L^1(\partial \Omega)}+ 
  \frac{\alpha}{2}\Vert f\Vert_{L^2(\Omega)}^2$ für alle 
  $v\in \BV(\Omega)\cap L^2(\Omega)$ und
  der Term $\frac{\alpha}{2}\Vert f\Vert_{L^2(\Omega)}^2$ konstant ist, haben
  beide Funktionale die gleichen Minimierer in $\BV(\Omega)\cap L^2(\Omega)$.

  Vlt so In Bar wird formuliert mit .. für g in L2. Mit f=alpha g bla bla und
  bla bla sind die Probleme äquivalent
\end{remark}
...
\begin{proof}
  ...
  \todo[inline]{TODO besser zitieren, Rellich Kondrachov erwähnen oder sogar
  nachrechnen?}
